...
#include "Common\CmnHdr.h"     /* See Appendix A. */
#include <WindowsX.h>
#include <tchar.h>
#include "Resource.h"
#include "injecttion_dll/injecttion_dll.h"
...
int WINAPI _tWinMain(HINSTANCE hInstExe, HINSTANCE, PTSTR pszCmdLine, int) {
    ...
    HWND desktop_listview = GetDestopListViewHandle();
    HWND hWndLV = GetFirstChild(GetFirstChild(
        FindWindow(TEXT("ProgMan"), NULL)));

    hWndLV = GetDestopListViewHandle();
	
    TCHAR szWndName[128] = { 0 };
    GetWindowText(hWndLV, szWndName, _countof(szWndName));
    ...
    HWND hWndDIPS = FindWindow(NULL, TEXT("TestDialog"));
