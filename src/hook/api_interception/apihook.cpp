...
#include "apihook.h"
#include "common_header\Toolhelp.h"
#include <StrSafe.h>
...
CAPIHook::CAPIHook(PSTR pszCalleeModName, PSTR pszFuncName, PROC pfnHook) {
    ...
    fprintf(stdout, "CAPIHook(%s, %s, %p)\n", pszCalleeModName,pszFuncName,pfnHook);
         ...
         for (; pThunk->u1.Function; pThunk++) {
            ...
            if (bFound) {
               if (!WriteProcessMemory(GetCurrentProcess(), ppfn, &pfnNew, 
                  ...
                  fprintf(stderr, "Line(%d) WriteProcessMemory GetLastError()=%d\n",__LINE__, GetLastError());
                  DWORD dwOldProtect;
                  if (VirtualProtect(ppfn, sizeof(pfnNew), PAGE_WRITECOPY, 
                    ...
                    fprintf(stderr, "Line(%d) WriteProcessMemory GetLastError()=%d\n",__LINE__, GetLastError());
                     VirtualProtect(ppfn, sizeof(pfnNew), dwOldProtect, 
                        &dwOldProtect);
                    fprintf(stderr, "Line(%d) VirtualProtect GetLastError()=%d\n",__LINE__, GetLastError());
                  }
               }
...
HMODULE WINAPI CAPIHook::LoadLibraryA(PCSTR pszModulePath) {
   ...
   char msg[1024] = {0};
   sprintf_s(msg, "LoadLibraryA(%s)\n", pszModulePath);
   SndCopyDataToWndA(msg);
...
HMODULE WINAPI CAPIHook::LoadLibraryW(PCWSTR pszModulePath) {
   wchar_t msg[1024] = {0};
   swprintf_s(msg, L"LoadLibraryW(%s)\n", pszModulePath);
   SndCopyDataToWndW(msg);
...
HMODULE WINAPI CAPIHook::LoadLibraryExA(PCSTR pszModulePath, 
   ...
   char msg[1024] = {0};
    sprintf_s(msg, "LoadLibraryExA(%s,%p,%d)\n", pszModulePath,hFile, dwFlags);
   SndCopyDataToWndA(msg);
...
HMODULE WINAPI CAPIHook::LoadLibraryExW(PCWSTR pszModulePath, 
   HANDLE hFile, DWORD dwFlags) {
   wchar_t msg[1024] = {0};
   swprintf_s(msg, L"LoadLibraryExW(%s,%p,%d)\n", pszModulePath,hFile, dwFlags);
   SndCopyDataToWndW(msg);
...
FARPROC WINAPI CAPIHook::GetProcAddress(HMODULE hmod, PCSTR pszProcName) {
      char msg[1024] = {0};
    sprintf_s(msg, "GetProcAddress(%p, %s)\n", hmod, pszProcName);
    SndCopyDataToWndA(msg);
